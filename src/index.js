...
function Login() {
    ...
    return <div className="w-screen h-screen bg-slate-900 flex items-center justify-center">
        <div className="p-8 bg-slate-700 rounded flex flex-col items-center shadow-white">
            ...
            <span className="mb-4 text-slate-200">{JSON.stringify(user)} - {JSON.stringify(loading)} - {JSON.stringify(error)}</span>
            ...
            <button className="p-2 bg-white mt-2 rounded" type="button" onClick={() => signInWithEmailAndPassword(username, password)}>
                Login
            </button>
        </div>
    </div>
